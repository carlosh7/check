...
| Duplicados + tramo 4 + auto-deploy | ✅ Completada | 12.44.808 | **P3-8**: 6 métodos duplicados en App eliminados (la sombreada nunca corría; guard de Swal fusionado) · **CVEs: 0 vulnerabilidades** (override uuid ≥11.1.1 para exceljs/gaxios/googleapis-common) · **tramo 4 ESLint**: 504→455 (26 handlers backend de-async con Express 5 capturando sync-throw, var→let seguros, initializers muertos fuera; se conservan documentados: require-await frontend/SDK=contrato, no-await-in-loop=patrón secuencial, 16 var con hoisting real) · **C6-14 auto-deploy**: modo script en /api/deploy/webhook (DEPLOY_SCRIPT_PATH) + scripts/vps-redeploy.sh — activación requiere decisión del operador (socket docker). 300/301 tests. |
