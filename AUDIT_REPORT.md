...
| P3-7 | P3 | **Hallazgos de código muerto con posible impacto de producto** (detectados en tramo 3 ESLint v12.44.806, pendiente decisión de producto): permisos `isSelf/canEdit/canRemoveGroup/canRemoveEvent` calculados pero nunca usados en la render de usuarios (`app.js:4268-4271` → la tabla de usuarios podría no estar aplicando permisos como se espera) · chips de eventos/staff construidos y nunca renderizados en la vista de clientes (`app.js:1722/1729`) · `usableH` calculado e ignorado en el generador de auditorios de seat-layouts (posible desborde vertical del mapa de asientos) · filtros `userEvents/userGroupIds` muertos en vista de grupos | 🟡 **DOCUMENTADO v12.44.806** — requiere revisión de producto, no auto-fijable sin decidir comportamiento |
