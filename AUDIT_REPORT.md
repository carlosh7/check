...
| P2-3 | P2 | CSP con unsafe-inline + frameSrc * | 🟡 **PARCIAL — cierre ligado a la modularización de app.js**: scriptSrc ya sin 'unsafe-inline' (v12.44.804, scripts static externalizados). **Restricción arquitectónica documentada (v12.44.806)**: el resto (`scriptSrcAttr`/`styleSrc`) no puede quitarse solo migrando los ~460 handlers estáticos de `app-shell.html` — `app.js` (20k líneas) y las páginas generan **onclick/onchange dinámicamente en strings HTML** (p.ej. `portal.js:126`, cientos de render functions) que se inyectan por `innerHTML`; hasta modularizar esos renders, quitar `script-src-attr` rompería la app en producción |
...
| P3-7 | P3 | **Hallazgos de código muerto con posible impacto de producto** (detectados en tramo 3 ESLint v12.44.806): permisos `isSelf/canEdit/canRemoveGroup/canRemoveEvent` calculados pero nunca usados en la render de usuarios · chips de eventos/staff construidos y nunca renderizados en la vista de clientes (`app.js:1722/1729`) · `usableH` calculado e ignorado en el generador de auditorios de seat-layouts · filtros `userEvents/userGroupIds` muertos en vista de grupos | ✅ **RESUELTOS v12.44.807** (decisión del operador: aplicar lo recomendado): chips y filtros muertos eliminados (el render real usa la versión inline con iconos; el filtrado real usa `userGroupId`) · permisos de la tabla de usuarios verificados server-side (PUT/DELETE = ADMIN/PRODUCTOR, role/group = ADMIN) — no hay agujero de seguridad, las variables muertas se retiraron · **seat-layouts ahora valida que las filas quepan en el largo útil de la sala** y responde 400 con mensaje claro ("Las filas no caben en la sala…") en lugar de dibujar el plano desbordado |
