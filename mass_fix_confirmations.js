...
<div style="font-family: 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #020617; padding: 40px 20px; color: #f8fafc !important; margin: 0; color-scheme: dark; -webkit-font-smoothing: antialiased;">
    <div style="max-width: 600px; margin: 0 auto; background-color: #0f172a; border-radius: 24px; border: 1px solid rgba(255, 255, 255, 0.08); overflow: hidden; box-shadow: 0 20px 50px rgba(0,0,0,0.3);">
        ...
        <div style="padding: 40px;">
            <div style="color: #94a3b8 !important; line-height: 1.8; font-size: 16px;">
                ${content}
            </div>
            ${actionText ? `
            ...
            <div style="margin-top: 40px; padding-top: 30px; border-top: 1px solid rgba(255, 255, 255, 0.08); text-align: center;">
                <p style="color: #64748b !important; font-size: 13px; margin: 0;">
                    Atentamente,<br>
                    <strong style="color: #f8fafc !important; font-size: 15px;">El Equipo de Check Pro</strong>
                </p>
...
const body = premiumStyle(
    ...
    <p>Tu participación en <strong>{{event_name}}</strong> ha sido registrada con éxito. Estamos preparando todo para brindarte la mejor experiencia.</p>
    <div style="background: rgba(255,255,255,0.03); border-radius: 16px; padding: 25px; margin: 30px 0; border: 1px solid rgba(255,255,255,0.05);">
        <p style="margin: 0 0 10px 0; color: #f8fafc !important;"><strong>Evento:</strong> {{event_name}}</p>
        <p style="margin: 0 0 10px 0; color: #f8fafc !important;"><strong>Fecha:</strong> {{event_date}}</p>
        <p style="margin: 0; color: #f8fafc !important;"><strong>Lugar:</strong> {{event_location}}</p>
    </div>
    <p>Puedes acceder a tu invitación y más detalles en el siguiente enlace.</p>`,
    'Ver Invitación',
    '{{login_url}}'
);

// Update [Modelo] specifically
const s1 = db.prepare("UPDATE email_templates SET body = ?, updated_at = ? WHERE name LIKE '%Confirmación%'");
s1.run(body, new Date().toISOString());

// Update ALL event confirmations
const s2 = db.prepare("UPDATE event_email_templates SET body = ?, updated_at = ? WHERE name LIKE '%Confirmación%'");
s2.run(body, new Date().toISOString());

db.close();
console.log("ALL confirmation templates updated with premium style and dark compatibility.");
