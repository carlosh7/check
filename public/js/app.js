...
const App = window.App = {
    ...
    _normalize: function(text) {
        // Usa Utils del módulo
        if (typeof Constants !== 'undefined' && Constants.Utils) {
            return Constants.Utils.normalize(text);
        }
        // Fallback
        if (!text) return '';
...
<div class="w-3 h-3 rounded-lg bg-gradient-to-br from-[#0ba5ec] to-blue-500/20 flex items-center justify-center flex-shrink-0 border border-[#0ba5ec]">
    ...
    // ── Planos de Sala (Seat Maps) ──

    loadSeatLayouts: async function() {
        const eId = this.state.event?.id;
        if (!eId) return;
        try {
            const layouts = await this.fetchAPI('/seat-layouts/' + eId);
            const container = document.getElementById('seat-layouts-list');
            if (!container) return;
            if (!layouts || layouts.length === 0) {
                container.innerHTML = '<p class="text-slate-500 italic col-span-3 text-center py-8">Sin planos. Crea el primero!</p>';
                return;
            }
    ...
    openSeatLayoutModal: function() {
        ...
        document.getElementById('seat-cols').value = 8;
    ...
    closeSeatEditor: function() {
        document.getElementById('seat-layout-editor').classList.add('hidden');
        document.getElementById('seat-layouts-list').classList.remove('hidden');
        delete this._editingSeatLayoutId;
        delete this._seatRenderCache;
    ...
    saveSeatLayout: async function() {
        ...
        try {
            if (this._editingSeatLayoutId) {
                ...
                await this.fetchAPI('/seat-layouts/' + eId, { method: 'POST', body: JSON.stringify({ name: name, config: config }) });
            }
                ...
                <div class="p-6 overflow-y-auto flex-1 space-y-6">
                    ...
                    <div class="border-t border-[var(--border)] pt-4">
                        <h4 class="text-sm font-bold text-[var(--text-main)] mb-3 flex items-center gap-2">
                            <span class="material-symbols-outlined text-sm text-violet-400">send</span> Configuraci├│n SMTP (Enviar)
                        </h4>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                            ...
                            <div>
                                <label class="text-xs font-bold uppercase text-[var(--text-secondary)] mb-1 block">Contrase├▒a SMTP</label>
                                <input type="password" id="email-smtp-pass" value="${account.smtp_password === '***' ? '' : account.smtp_password || ''}" class="input-field w-full" placeholder="${account.smtp_password === '***' ? 'ÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇó' : 'Contrase├▒a o app password'}">
                            </div>
                        </div>
                        <div class="mt-3">
                    ...
                    <div class="border-t border-[var(--border)] pt-4">
                        <h4 class="text-sm font-bold text-[var(--text-main)] mb-3 flex items-center gap-2">
                            <span class="material-symbols-outlined text-sm text-blue-400">inbox</span> Configuraci├│n IMAP (Recibir)
                        </h4>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                            ...
                            <div>
                                <label class="text-xs font-bold uppercase text-[var(--text-secondary)] mb-1 block">Contrase├▒a IMAP</label>
                                <input type="password" id="email-imap-pass" value="${account.imap_password === '***' ? '' : account.imap_password || ''}" class="input-field w-full" placeholder="${account.imap_password === '***' ? 'ÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇó' : 'Contrase├▒a'}">
                            </div>
                        </div>
                        <div class="mt-3">
                            <label class="flex items-center gap-2 text-sm text-[var(--text-main)]">
                    ...
                    <div class="border-t border-[var(--border)] pt-4">
                        ...
                        <div class="mt-3">
                            <label class="text-xs font-bold uppercase text-[var(--text-secondary)] mb-1 block">L├¡mite diario de emails</label>
    ...
    showEmailSetupHelp: function() {
        Swal.fire({
            title: 'Instructivo de Configuraci├│n',
            html: `
            <div class="text-left space-y-4 max-h-[60vh] overflow-y-auto">
                <div class="p-4 bg-blue-500/10 rounded-xl border border-blue-500/20">
                    <h4 class="font-bold text-blue-400 mb-2">­ƒôº GMAIL</h4>
                    <ol class="text-xs text-slate-300 space-y-1 list-decimal list-inside">
                        <li>Ve a tu cuenta de Google ÔåÆ Seguridad</li>
                        <li>Habilita "Verificaci├│n en dos pasos"</li>
                        <li>Ve a "Contrase├▒as de aplicaciones"</li>
                        <li>Genera una nueva contrase├▒a de 16 caracteres</li>
                        <li>Usa esa contrase├▒a en "Contrase├▒a SMTP"</li>
                        <li>Para IMAP usa tu contrase├▒a normal de Google</li>
                    </ol>
                </div>
                
                <div class="p-4 bg-orange-500/10 rounded-xl border border-orange-500/20">
                    <h4 class="font-bold text-orange-400 mb-2">­ƒôº OUTLOOK / HOTMAIL</h4>
                    <ol class="text-xs text-slate-300 space-y-1 list-decimal list-inside">
                        <li>Ve a account.microsoft.com</li>
                        <li>Seguridad avanzada</li>
                        <li>Habilita autenticaci├│n MFA</li>
                        <li>Contrase├▒as de aplicaci├│n ÔåÆ Nueva</li>
                        <li>Usa esa contrase├▒a</li>
                    </ol>
                </div>
                
                <div class="p-4 bg-green-500/10 rounded-xl border border-green-500/20">
                    <h4 class="font-bold text-green-400 mb-2">­ƒôº OTROS PROVEEDORES</h4>
                    <p class="text-xs text-slate-300">Busca en la ayuda de tu proveedor: "configuraci├│n SMTP/IMAP"</p>
                    <p class="text-xs text-slate-400 mt-2">Ejemplo gen├®rico:</p>
    ...
    testSmtpConnection: async function() {
        ...
        try {
            ...
            if (result.success) {
                Swal.fire('Ô£ô ├ëxito', 'Conexi├│n SMTP exitosa', 'success');
            } else {
                Swal.fire('Error', result.error || 'Error de conexi├│n SMTP', 'error');
            }
        } catch (e) {
    ...
    testImapConnection: async function() {
        ...
        try {
            ...
            if (result.success) {
                Swal.fire('Ô£ô ├ëxito', 'Conexi├│n IMAP exitosa', 'success');
            } else {
                Swal.fire('Error', result.error || 'Error de conexi├│n IMAP', 'error');
            }
        } catch (e) {
            Swal.fire('Error', 'No se pudo conectar al servidor', 'error');
        }
    ...
    saveEmailAccount: async function() {
        ...
        try {
            if (id) {
                // Al editar, si la password viene vac├¡a y el placeholder era '***', no enviarla
            ...
            if (el) el.classList.add('hidden');
            this.loadEmailAccounts();
            Swal.fire('Ô£ô ├ëxito', 'Cuenta guardada correctamente', 'success');
        } catch (e) {
    ...
    testEmailAccount: async function(accountId, type) {
        Swal.fire({ title: 'Probando conexi├│n...', text: 'Por favor espera', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
        ...
        try {
            ...
            if (result.success) {
                Swal.fire('Ô£ô ├ëxito', result.message || 'Conexi├│n exitosa', 'success');
            } else {
                Swal.fire('Ô£ù Error', result.error || 'Error en la conexi├│n', 'error');
            }
        } catch (e) {
            Swal.fire('Error', 'No se pudo probar la conexi├│n', 'error');
        }
    },
    ...
    deleteEmailAccount: async function(accountId) {
        ...
        if (result.isConfirmed) {
            try {
                ...
                this.loadEmailAccounts();
                Swal.fire('Ô£ô Eliminado', 'Cuenta eliminada', 'success');
            } catch (e) {
    ...
    downloadAttachment: async function(attIndex, accountId, folder, uid) {
        try {
            const res = await this.fetchAPI(`/email/mailbox/attachment/${uid}?account_id=${accountId}&folder=${folder}&index=${attIndex}`);
            if (res?.url) {
                window.open(res.url, '_blank');
            } else {
                Swal.fire('Info', 'Descarga iniciada', 'info');
            }
        } catch (e) {
    ...
    seedAllTemplates: async function() {
        try {
            ...
            if (result.success) {
                Swal.fire('Exito', 'Plantillas creadas correctamente', 'success');
                this.loadEmailTemplates();
            }
        } catch (e) {
            Swal.fire('Error', e.message || 'No se pudieron crear las plantillas', 'error');
        }
    },
    ...
    sendMailing: async function() {
        ...
        try {
            ...
            Swal.fire('Ô£ô ├ëxito', 'Mailing en progreso', 'success');
    ...
    loadMailingCampaigns: async function() {
        ...
        try {
            ...
            if (!campaigns || campaigns.length === 0) {
                container.innerHTML = `<p class="text-center py-8 text-slate-500 text-sm">No hay campa├▒as para este evento</p>`;
                return;
            }
...
App.toggleValidateAttendance = async function(clientId) {
    ...
    try {
        ...
        // Si es check-in (validated=1), verificar config de gafete
        if (newValidated === 1) {
            try {
