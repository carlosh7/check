...
function loadPortalPolls(){
  ...
  fetch(API+'/polls/public/'+eId+'/active').then(function(r){return r.json()}).then(function(polls){
    ...
    c.innerHTML=polls.map(function(p){
      ...
      return '<div class="card" style="margin:12px 0"><div class="card-title">📊 '+escHtml(p.title)+'</div>'
        +(p.description?'<p style="font-size:12px;color:rgba(255,255,255,0.5);margin-bottom:12px">'+escHtml(p.description)+'</p>':'')
        +optionsHtml
        +'<button class="vote-btn" data-pollid="'+p.id+'" style="margin-top:12px;width:100%;padding:10px;border:none;border-radius:10px;background:linear-gradient(135deg,#7c3aed,#a78bfa);color:#fff;font-size:13px;font-weight:600;cursor:pointer" onclick="votePoll(this)">Votar</button></div>';
    }).join('');
  }).catch(function(e){console.error('Polls load error:',e);document.getElementById('portal-polls-list').innerHTML='<p style="font-size:13px;color:#ef4444;text-align:center;padding:20px 0">Error al cargar encuestas</p>'});
}
    ...
    c.innerHTML='<div style="display:flex;flex-direction:column;gap:4px">'
      +entries.map(function(e,i){
        ...
        return '<div style="display:flex;align-items:center;gap:10px;padding:10px;background:'+(isMe?'rgba(124,58,237,0.1)':'transparent')+';border-radius:10px;border:1px solid '+(isMe?'rgba(124,58,237,0.2)':'rgba(255,255,255,0.04)')+'">'
          ...
          +(e.organization?'<span style="font-size:11px;color:rgba(255,255,255,0.4);display:block">'+escHtml(e.organization)+'</span>':'')+'</div>'
          +'<span style="font-size:15px;font-weight:700;color:#a78bfa">'+e.points+' pts</span>'
          +'</div>';
      }).join('')
      +'</div>';
  }).catch(function(){});
  ...
  fetch(API+'/leaderboard/'+eId+'/guest/'+gId+'/badges').then(function(r){return r.json()}).then(function(badges){
    ...
    c.innerHTML=badges.map(function(b){
      return '<div style="display:flex;align-items:center;gap:10px;padding:10px;background:rgba(255,255,255,0.03);border-radius:10px;margin:4px 0">'
        ...
        +'<div><span style="font-size:13px;font-weight:600;color:#e2e8f0">'+escHtml(b.name)+'</span>'
        +(b.description?'<span style="font-size:11px;color:rgba(255,255,255,0.4);display:block">'+escHtml(b.description)+'</span>':'')+'</div></div>';
    }).join('');
  }).catch(function(){});
}
...
function loadNetworking() {
  ...
  fetch(API + '/networking/' + eventId + '/guest/' + guestId).then(function(r) { return r.json(); }).then(function(conns) {
    ...
    c.innerHTML = conns.map(function(n) {
      return '<div onclick="openNetProfile(\'' + n.connected_guest_id + '\')" style="display:flex;align-items:center;gap:10px;padding:10px;background:rgba(255,255,255,0.03);border-radius:10px;margin:4px 0;cursor:pointer">'
        ...
        + '<div style="flex:1"><span style="font-size:13px;font-weight:600;color:#e2e8f0">' + escHtml(n.connected_name) + '</span>'
        + '<span style="font-size:11px;color:rgba(255,255,255,0.4);display:block">' + escHtml(n.connected_email || n.connected_org || '') + '</span></div>'
        + '<span style="font-size:16px;color:rgba(255,255,255,0.3)">›</span></div>';
    }).join('');
  }).catch(function() {});
  ...
  fetch(API + '/networking/' + eventId + '/guest/' + guestId + '/suggestions').then(function(r) { return r.json(); }).then(function(sugs) {
    ...
    box.innerHTML = sugs.slice(0, 5).map(function(sg) {
      return '<div style="display:flex;align-items:center;gap:10px;padding:10px;background:rgba(255,255,255,0.03);border-radius:10px;margin:4px 0">'
        + '<span style="font-size:18px">🤝</span>'
        + '<div style="flex:1;min-width:0"><span style="font-size:13px;font-weight:600;color:#e2e8f0">' + escHtml(sg.name) + '</span>'
        + '<span style="font-size:11px;color:rgba(255,255,255,0.4);display:block">' + escHtml(sg.organization || sg.position || '') + (sg.shared_connections ? ' · ' + sg.shared_connections + ' conexiones en común' : '') + '</span></div>'
        + (sg.qr_token ? '<button onclick="connectSuggestion(\'' + sg.qr_token + '\',\'' + escHtml(sg.name).replace(/'/g,'') + '\')" style="padding:6px 12px;border-radius:8px;border:none;background:rgba(16,185,129,0.15);color:#34d399;font-size:11px;font-weight:700;cursor:pointer">Conectar</button>' : '')
        + '</div>';
    }).join('');
  }).catch(function() {});
}
  ...
  fetch(API + '/album/' + eId).then(function(r) { return r.json(); }).then(function(photos) {
    ...
    c.innerHTML = photos.map(function(p) {
      return '<div style="position:relative;border-radius:8px;overflow:hidden;aspect-ratio:1;background:rgba(255,255,255,0.03)"><img src="' + p.url + '" style="width:100%;height:100%;object-fit:cover;cursor:pointer" onclick="viewPhoto(\'' + p.url + '\',\'' + escJs(p.caption||'') + '\')">'
        + (p.caption ? '<div style="position:absolute;bottom:0;left:0;right:0;padding:4px 6px;background:linear-gradient(transparent,rgba(0,0,0,0.7));font-size:10px;color:rgba(255,255,255,0.8)">' + escHtml(p.caption) + '</div>' : '')
        + '</div>';
    }).join('');
  }).catch(function() {});
}
...
async function load(){
  ...
  try{
    ...
    document.getElementById('loading').classList.add('hidden');
    document.getElementById('main').innerHTML='<div style="padding:40px;text-align:center"><h2 style="color:#ef4444">Portal no disponible</h2><p style="color:rgba(255,255,255,0.4);margin-top:8px">No se pudo cargar tu información.</p></div>';
    document.getElementById('main').style.display='block'
  }
}
load();
    ...
    fetch(API + '/networking/profile/' + guestId + '?event_id=' + eventId).then(function(r){ return r.json(); }).then(function(prof) {
      fetch(API + '/networking/' + eventId + '/guest/' + guestId + '/mutual?other=' + myId).then(function(r){ return r.json(); }).then(function(mutual) {
        ...
        ov.innerHTML = '<div style="background:#111827;border:1px solid rgba(255,255,255,0.08);border-radius:16px;max-width:380px;width:100%;padding:24px;color:#e2e8f0">'
          + '<div style="text-align:center;margin-bottom:12px"><div style="width:56px;height:56px;border-radius:50%;background:rgba(16,185,129,0.15);display:flex;align-items:center;justify-content:center;margin:0 auto 8px;font-size:26px">👤</div>'
          + '<h3 style="font-size:16px;font-weight:800;margin:0">' + escHtml(prof.name || '') + '</h3>'
          + '<p style="font-size:12px;color:rgba(255,255,255,0.5);margin:2px 0">' + escHtml(prof.position || '') + (prof.organization ? ' · ' + escHtml(prof.organization) : '') + '</p>'
          + (prof.email ? '<p style="font-size:12px;color:#34d399;margin:4px 0 0">' + escHtml(prof.email) + '</p>' : '') + '</div>'
          + '<div style="display:flex;gap:8px;margin:12px 0">'
          + '<div style="flex:1;text-align:center;background:rgba(255,255,255,0.04);border-radius:10px;padding:8px"><p style="font-size:18px;font-weight:800;margin:0">' + (prof.connections || 0) + '</p><p style="font-size:10px;color:rgba(255,255,255,0.4)">CONEXIONES</p></div>'
          + '<div style="flex:1;text-align:center;background:rgba(255,255,255,0.04);border-radius:10px;padding:8px"><p style="font-size:18px;font-weight:800;margin:0">' + mutualNames.length + '</p><p style="font-size:10px;color:rgba(255,255,255,0.4)">EN COMÚN</p></div></div>'
          + mutualHtml
          + '<p style="font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:1px;color:rgba(255,255,255,0.35);margin:14px 0 4px">Sesiones</p>' + sessionsHtml
          + '<button onclick="this.closest(\'#net-profile-overlay\').remove()" style="width:100%;margin-top:16px;padding:10px;border-radius:10px;border:none;background:rgba(255,255,255,0.06);color:#e2e8f0;font-weight:700;cursor:pointer">Cerrar</button></div>';
