...
router.get('/:eventId', authMiddleware(['ADMIN', 'PRODUCTOR', 'ORGANIZER']), (req, res) => {
    try {
        const polls = db.prepare("SELECT * FROM polls WHERE event_id = ? ORDER BY created_at DESC").all(req.params.eventId);
...
router.post('/:eventId', authMiddleware(['ADMIN', 'PRODUCTOR']), (req, res) => {
    try {
        ...
        const { title, description, type, session_id, points, time_limit_seconds, correct_answer, options } = result.data;
        const id = uuidv4();
        ...
        if (options && Array.isArray(options)) {
            const insertOpt = db.prepare("INSERT INTO poll_options (id, poll_id, label, order_index, is_correct) VALUES (?, ?, ?, ?, ?)");
...
router.get('/:eventId/:pollId', authMiddleware(['ADMIN', 'PRODUCTOR', 'ORGANIZER']), (req, res) => {
    try {
        const poll = db.prepare("SELECT * FROM polls WHERE id = ? AND event_id = ?").get(req.params.pollId, req.params.eventId);
        ...
        const options = db.prepare("SELECT * FROM poll_options WHERE poll_id = ? ORDER BY order_index ASC").all(req.params.pollId);
...
router.put('/:eventId/:pollId', authMiddleware(['ADMIN', 'PRODUCTOR']), (req, res) => {
    try {
        const { title, description, type, session_id, points, time_limit_seconds, correct_answer, status } = req.body;
...
router.get('/:eventId/:pollId/results', authMiddleware(['ADMIN', 'PRODUCTOR', 'ORGANIZER']), (req, res) => {
    try {
        ...
        if (!poll) return res.status(404).json({ error: 'Encuesta no encontrada' });
        const options = db.prepare("SELECT * FROM poll_options WHERE poll_id = ? ORDER BY order_index ASC").all(req.params.pollId);
        const votes = db.prepare("SELECT pv.*, g.name as guest_name FROM poll_votes pv LEFT JOIN guests g ON g.id = pv.guest_id WHERE pv.poll_id = ? ORDER BY pv.voted_at DESC").all(req.params.pollId);
        ...
        if (poll.type === 'trivia' && poll.correct_answer) {
            const correctAnswers = typeof poll.correct_answer === 'string' ? JSON.parse(poll.correct_answer) : poll.correct_answer;
            votes.forEach(function(v) {
...
router.get('/public/:eventId/active', (req, res) => {
    try {
        const polls = db.prepare("SELECT id, event_id, session_id, title, description, type, points, time_limit_seconds FROM polls WHERE event_id = ? AND status = 'active' ORDER BY created_at DESC").all(req.params.eventId);
        polls.forEach(function(p) {
...
router.post('/public/:pollId/vote', (req, res) => {
    try {
        ...
        const { guest_token, option_id, answer_text } = parsed.data;
        const guest = db.prepare("SELECT id, event_id FROM guests WHERE qr_token = ?").get(guest_token);
        if (!guest) return res.status(401).json({ error: 'Invitado no encontrado' });
        const poll = db.prepare("SELECT * FROM polls WHERE id = ? AND status = 'active'").get(req.params.pollId);
        if (!poll) return res.status(404).json({ error: 'Encuesta no activa o no encontrada' });
        // Verificar si ya votó
        const existingVote = db.prepare("SELECT id FROM poll_votes WHERE poll_id = ? AND guest_id = ?").get(req.params.pollId, guest.id);
        ...
        try { const io = require('../../src/socket').getIO(); if (io) { io.to(poll.event_id).emit('poll_updated', { pollId: poll.id, pollTitle: poll.title }); } } catch(e) {}
        // Sumar puntos al leaderboard
        if (poll.points > 0) {
            const existing = db.prepare("SELECT id FROM leaderboard WHERE event_id = ? AND guest_id = ?").get(guest.event_id, guest.id);
            if (existing) {
...
router.get('/public/:pollId/results', (req, res) => {
    try {
        const poll = db.prepare("SELECT id, title, type, correct_answer FROM polls WHERE id = ? AND status = 'closed'").get(req.params.pollId);
        if (!poll) return res.status(404).json({ error: 'Resultados no disponibles' });
        if (poll.correct_answer) { try { poll.correct_answer = JSON.parse(poll.correct_answer); } catch(e) { poll.correct_answer = null; } }
        const options = db.prepare("SELECT id, label, is_correct FROM poll_options WHERE poll_id = ? ORDER BY order_index ASC").all(req.params.pollId);
        const totalVotes = db.prepare("SELECT COUNT(*) as count FROM poll_votes WHERE poll_id = ?").get(req.params.pollId).count;
        const votes = db.prepare("SELECT option_id FROM poll_votes WHERE poll_id = ?").all(req.params.pollId);
        const counts = {};
...
function checkBadges(eventId, guestId) {
    try {
        const badges = db.prepare("SELECT * FROM badges WHERE event_id = ?").all(eventId);
        const guestPoints = db.prepare("SELECT points FROM leaderboard WHERE event_id = ? AND guest_id = ?").get(eventId, guestId);
        const pollCount = db.prepare("SELECT COUNT(*) as c FROM poll_votes pv JOIN polls p ON p.id = pv.poll_id WHERE p.event_id = ? AND pv.guest_id = ?").get(eventId, guestId).c;
        badges.forEach(function(b) {
            const earned = db.prepare("SELECT id FROM guest_badges WHERE badge_id = ? AND guest_id = ?").get(b.id, guestId);
            if (earned) return;
            let criteria = {};
            ...
            if (award) {
                ...
                if (b.points_reward > 0) {
                    const lb = db.prepare("SELECT id FROM leaderboard WHERE event_id = ? AND guest_id = ?").get(eventId, guestId);
                    if (lb) {
