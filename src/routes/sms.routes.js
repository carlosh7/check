...
router.get('/api/whatsapp/settings', authMiddleware(['ADMIN']), (req, res) => {
    try {
        var settings = db.prepare("SELECT setting_key, setting_value FROM settings WHERE setting_key LIKE 'wa_%'").all();
        var result = {};
...
router.post('/api/whatsapp/settings', authMiddleware(['ADMIN']), (req, res) => {
    try {
        var { phone_number_id, access_token, from_phone, enabled } = req.body;
        var upsert = function(key, val) {
            var existing = db.prepare("SELECT setting_key FROM settings WHERE setting_key = ?").get(key);
            if (existing) db.prepare("UPDATE settings SET setting_value = ? WHERE setting_key = ?").run(val, key);
            else db.prepare("INSERT INTO settings (setting_key, setting_value) VALUES (?, ?)").run(key, val);
...
router.post('/api/whatsapp/send', authMiddleware(['ADMIN']), async (req, res) => {
    try {
        var { to, message } = req.body;
        if (!to || !message) return res.status(400).json({ error: 'Teléfono y mensaje requeridos' });
        var phoneId = db.prepare("SELECT setting_value FROM settings WHERE setting_key = 'wa_phone_number_id'").get();
        var token = db.prepare("SELECT setting_value FROM settings WHERE setting_key = 'wa_access_token'").get();
        if (!phoneId || !token) return res.status(400).json({ error: 'WhatsApp no configurado' });

        var response = await fetch('https://graph.facebook.com/v18.0/' + phoneId.setting_value + '/messages', {
            method: 'POST',
...
router.post('/api/whatsapp/send-to-guest/:guestId', authMiddleware(['ADMIN', 'PRODUCTOR']), async (req, res) => {
    try {
        ...
        var event = db.prepare("SELECT name, date, location FROM events WHERE id = ?").get(guest.event_id);
        var msg = 'Hola ' + (guest.name || '') + '! Te esperamos en ' + (event ? event.name : 'el evento') + '.';
