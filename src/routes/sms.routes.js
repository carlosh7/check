...
router.get('/api/sms/settings', authMiddleware(['ADMIN']), (req, res) => {
    try {
        const settings = db.prepare("SELECT setting_key, setting_value FROM settings WHERE setting_key LIKE 'sms_%'").all();
        const result = {};
...
router.post('/api/sms/send', authMiddleware(['ADMIN']), async (req, res) => {
    try {
        ...
        const accountSid = db.prepare("SELECT setting_value FROM settings WHERE setting_key = 'sms_account_sid'").get();
        const authToken = db.prepare("SELECT setting_value FROM settings WHERE setting_key = 'sms_auth_token'").get();
...
router.post('/api/sms/send-to-guest/:guestId', authMiddleware(['ADMIN', 'PRODUCTOR']), async (req, res) => {
    try {
        const guest = db.prepare("SELECT id, name, phone, event_id FROM guests WHERE id = ?").get(req.params.guestId);
        ...
        const event = db.prepare("SELECT name, date, location FROM events WHERE id = ?").get(guest.event_id);
...
router.get('/api/whatsapp/settings', authMiddleware(['ADMIN']), (req, res) => {
    try {
        const settings = db.prepare("SELECT setting_key, setting_value FROM settings WHERE setting_key LIKE 'wa_%'").all();
        const result = {};
...
router.post('/api/whatsapp/settings', authMiddleware(['ADMIN']), (req, res) => {
    try {
        const { phone_number_id, access_token, from_phone, enabled } = req.body;
        const upsert = function(key, val) {
            const existing = db.prepare("SELECT setting_key FROM settings WHERE setting_key = ?").get(key);
            if (existing) db.prepare("UPDATE settings SET setting_value = ? WHERE setting_key = ?").run(val, key);
            else db.prepare("INSERT INTO settings (setting_key, setting_value) VALUES (?, ?)").run(key, val);
...
router.post('/api/whatsapp/send', authMiddleware(['ADMIN']), async (req, res) => {
    try {
        const { to, message } = req.body;
        if (!to || !message) return res.status(400).json({ error: 'Teléfono y mensaje requeridos' });
        const phoneId = db.prepare("SELECT setting_value FROM settings WHERE setting_key = 'wa_phone_number_id'").get();
        const token = db.prepare("SELECT setting_value FROM settings WHERE setting_key = 'wa_access_token'").get();
        if (!phoneId || !token) return res.status(400).json({ error: 'WhatsApp no configurado' });

        const response = await fetch('https://graph.facebook.com/v18.0/' + phoneId.setting_value + '/messages', {
            method: 'POST',
