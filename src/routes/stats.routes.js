...
router.get('/analytics', authMiddleware(['ADMIN']), (req, res) => {
    try {
        ...
        const totalUsers = db.prepare("SELECT COUNT(*) as c FROM users WHERE status = 'APPROVED'").get().c;

        const trend = db.prepare("SELECT date(created_at) as day, COUNT(*) as c FROM guests WHERE created_at >= ? GROUP BY day ORDER BY day ASC").all(since);
        const topEvents = db.prepare("SELECT e.id, e.name, COUNT(g.id) as c FROM events e LEFT JOIN guests g ON g.event_id = e.id GROUP BY e.id ORDER BY c DESC LIMIT 10").all();
        const monthly = db.prepare("SELECT strftime('%Y-%m', created_at) as month, COUNT(*) as c FROM guests GROUP BY month ORDER BY month ASC LIMIT 12").all();
...
router.get('/stats/:eventId', authMiddleware(), (req, res) => {
    try {
        ...
        const mergedOrgs = Object.entries(orgMap).sort((a, b) => b[1] - a[1]).map(([organization, count]) => ({ organization, count }));
        const orgDistribution = mergedOrgs.slice(0, 5);
...
router.post('/db/maintenance', authMiddleware(['ADMIN']), (req, res) => {
    try {
        ...
        if (action === 'stats') {
            ...
            const indexCount = db.prepare("SELECT COUNT(*) as c FROM sqlite_master WHERE type='index'").get();
            result.stats = {
        ...
        if (action === 'quick_optimize' || action === 'all') {
            ...
            const deleted = db.prepare("DELETE FROM webhook_logs WHERE created_at < ?").run(old);
...
router.get('/reports/:eventId', authMiddleware(['ADMIN', 'PRODUCTOR']), (req, res) => {
    try {
        ...
        const event = db.prepare("SELECT name, date, location FROM events WHERE id = ?").get(eId);
        if (!event) return res.status(404).json({ error: 'Evento no encontrado' });

        const dbs = getDbsForEvent(eId);
        const total = querySum(dbs, "SELECT COUNT(*) as c FROM guests WHERE event_id = ?", [eId]);
        const checked = querySum(dbs, "SELECT COUNT(*) as c FROM guests WHERE event_id = ? AND checked_in = 1", [eId]);
        const pending = total - checked;

        const byOrg = queryAll(dbs, "SELECT organization, COUNT(*) as c FROM guests WHERE event_id = ? AND organization IS NOT NULL AND organization != '' GROUP BY organization ORDER BY c DESC LIMIT 5", [eId]);
        const byHour = queryAll(dbs, "SELECT strftime('%H', checkin_time) as h, COUNT(*) as c FROM guests WHERE event_id = ? AND checked_in = 1 AND checkin_time IS NOT NULL GROUP BY h ORDER BY c DESC LIMIT 3", [eId]);
        const byDay = queryAll(dbs, "SELECT date(created_at) as d, COUNT(*) as c FROM guests WHERE event_id = ? GROUP BY d ORDER BY d ASC", [eId]);

        const peakHour = byHour.length > 0 ? byHour[0].h + ':00' : 'N/A';
        const topOrg = byOrg.length > 0 ? byOrg[0].organization : 'N/A';
        const conversionRate = total > 0 ? Math.round((checked / total) * 100) : 0;
        ...
        if (checked > 0) {
            const attendeesPerOrg = byOrg.length;
...
router.get('/bi/dashboard', authMiddleware(['ADMIN']), (req, res) => {
    try {
        ...
        const totalChecked = db.prepare("SELECT COUNT(*) as c FROM guests WHERE checked_in = 1").get().c;
        const totalUsers = db.prepare("SELECT COUNT(*) as c FROM users WHERE status = 'APPROVED'").get().c;
        const conversionRate = totalGuests > 0 ? Math.round((totalChecked / totalGuests) * 100) : 0;

        // Trends (daily registrations)
        const dailyRegs = db.prepare("SELECT date(created_at) as d, COUNT(*) as c FROM guests WHERE created_at >= ? GROUP BY d ORDER BY d ASC").all(since);
        const dailyCheckins = db.prepare("SELECT date(checkin_time) as d, COUNT(*) as c FROM guests WHERE checked_in = 1 AND checkin_time >= ? GROUP BY d ORDER BY d ASC").all(since);

        // Top events
        const topEvents = db.prepare("SELECT e.id, e.name, e.date, COUNT(g.id) as guests, SUM(CASE WHEN g.checked_in = 1 THEN 1 ELSE 0 END) as checked FROM events e LEFT JOIN guests g ON g.event_id = e.id GROUP BY e.id ORDER BY guests DESC LIMIT 10").all();

        // Monthly comparison
        const monthly = db.prepare("SELECT strftime('%Y-%m', created_at) as m, COUNT(*) as c FROM guests GROUP BY m ORDER BY m ASC LIMIT 12").all();

        // Category distribution
        const catDist = db.prepare("SELECT c.name, COUNT(g.id) as count FROM guest_categories c LEFT JOIN guests g ON g.category_id = c.id GROUP BY c.id ORDER BY count DESC LIMIT 10").all();

        // Revenue stats (completed transactions)
        const revenue = db.prepare("SELECT SUM(amount) as total, COUNT(*) as txns, AVG(amount) as avg FROM transactions WHERE status = 'completed'").get();
        const revenueByMonth = db.prepare("SELECT strftime('%Y-%m', completed_at) as m, SUM(amount) as total, COUNT(*) as txns FROM transactions WHERE status = 'completed' AND completed_at >= ? GROUP BY m ORDER BY m ASC").all(since);

        // Hourly check-in distribution
        const hourlyDist = db.prepare("SELECT strftime('%H', checkin_time) as h, COUNT(*) as c FROM guests WHERE checked_in = 1 AND checkin_time IS NOT NULL GROUP BY h ORDER BY h ASC").all();

        // Polls stats (C11-01)
        const totalPolls = db.prepare("SELECT COUNT(*) as c FROM polls").get().c;
        const totalVotes = db.prepare("SELECT COUNT(*) as c FROM poll_votes").get().c;
        // Networking stats (C11-04)
        const totalConnections = db.prepare("SELECT COUNT(*) as c FROM networking_connections").get().c;
        // Certificates stats (C11-08)
        const totalCerts = db.prepare("SELECT COUNT(*) as c FROM guest_certificates").get().c;
...
router.get('/bi/trends', authMiddleware(['ADMIN']), (req, res) => {
    try {
        const compare = req.query.compare ? parseInt(req.query.compare) : 30;
        const current = req.query.period ? parseInt(req.query.period) : 30;
        const now = Date.now();

        const currentPeriod = new Date(now - current * 86400000).toISOString();
        const prevPeriod = new Date(now - (current + compare) * 86400000).toISOString();

        const currentGuests = db.prepare("SELECT COUNT(*) as c FROM guests WHERE created_at >= ?").get(currentPeriod).c;
        const prevGuests = db.prepare("SELECT COUNT(*) as c FROM guests WHERE created_at >= ? AND created_at < ?").get(prevPeriod, currentPeriod).c;
        const currentChecked = db.prepare("SELECT COUNT(*) as c FROM guests WHERE checked_in = 1 AND checkin_time >= ?").get(currentPeriod).c;
        const prevChecked = db.prepare("SELECT COUNT(*) as c FROM guests WHERE checked_in = 1 AND checkin_time >= ? AND checkin_time < ?").get(prevPeriod, currentPeriod).c;

        const calcGrowth = function(curr, prev) { return prev > 0 ? Math.round(((curr - prev) / prev) * 100) : curr > 0 ? 100 : 0; };
...
router.get('/health/system', authMiddleware(['ADMIN']), (req, res) => {
    try {
        let dbSize = 0;
        try { const size = db.prepare("PRAGMA page_count").get(); const page = db.prepare("PRAGMA page_size").get(); dbSize = (size?.page_count || 0) * (page?.page_size || 0); } catch(e) {}
...
router.get('/performance/logs', authMiddleware(['ADMIN']), (req, res) => {
    try {
        ...
        const logs = db.prepare("SELECT * FROM performance_logs ORDER BY created_at DESC LIMIT ?").all(limit);
        res.json(logs);
    } catch(err) { res.status(500).json({ error: err.message }); }
});
...
router.post('/system/backup', authMiddleware(['ADMIN']), async (req, res) => {
    try {
        const { createBackup } = require('../utils/webhooks');
